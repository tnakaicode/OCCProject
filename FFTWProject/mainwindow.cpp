...
MainWindow::MainWindow(int N, double fs, QString f_unit, QString t_unit, fftw_complex *in, fftw_complex *out, QWidget *parent)
    : QWidget(parent)
{
    // --- 時間波形 ---
    ...
    chart_time->axes(Qt::Horizontal).first()->setTitleText(QString("Time [%1]").arg(t_unit));
    ...
    {
        double freq = fromHz(k * fs / N, f_unit);
    ...
    chart_freq->axes(Qt::Horizontal).first()->setTitleText(QString("Frequency [%1]").arg(f_unit));
