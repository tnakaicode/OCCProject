...
1. **MSBuild開始**
2. ビルド成果物（exe, DLLなど）が存在しないことを確認
3. ソースをビルドし、exeを生成
4. **applocal.ps1の実行が必要と判断**
5. applocal.ps1実行 → vcpkgから必要なDLLをDebugにコピー
6. MSBuildはコピーされたDLLを「ビルド成果物」として記録
7. ビルド完了
...
1. **MSBuild開始**
2. ビルド成果物（exe, DLLなど）が既に存在し、最新であることを確認
3. **IncrementalCleanターゲットが前回の成果物リストと今回のリストを比較し、不要なDLLを削除**
   - ここでDebug内のDLLが消される
4. ソースの再ビルドやDLLコピーは不要と判断
5. applocal.ps1は実行されない（スキップ）
6. ビルド完了（DLLは消えたまま）
